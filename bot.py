    ...
    "mythic_spirit": {
        ...
        "seasons": {
            ...
            "1": {
                ...
                "description": (
                    "20-летний студент Харуто "
                    "Амакава погибает в ДТП. "
                    "Позже он приходит в себя "
                    "в незнакомом мире в теле "
                    "парня по имени Рио и "
            ...
            "2": {
                ...
                "description": (
                    "После побега из столицы "
                    "Бельтрама Рио вместе с "
                    "Селией и Аисией спасает "
                    "группу людей из Японии. "
                    "Среди них оказывается его "
                    "подруга детства Михару."
                ),
            },
        },
    },
...
def save_preview(
    ...
    DATA[
        "previews"
    ][anime_id][season_id][dub_id].setdefault(
        str(episode),
        {}
    )
...
async def send_anime_video(
    ...
    old_message_id = LAST_VIDEO_MESSAGES.get(
        user_id
    )
    ...
    caption = (
        ...
        f"🎞 Серия {episode} "
        f"из {anime['seasons'][season_id]['episodes']}\n"
    ...
    # -----------------------------------------------------
    # ОТПРАВЛЯЕМ ОДНО ВИДЕО С КНОПКАМИ
    # -----------------------------------------------------
    ...
    try:
        ...
        # Если сохранённое превью Telegram
        # не принимает, отправляем без него.
...
async def receive_video(
    ...
    await update.message.reply_text(
        ...
        "🖼 Если у видео есть Telegram-превью, "
        "оно также будет сохранено.",
...
async def delete_video_command(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE
):
    ...
    if len(context.args) != 5:
        ...
        await update.message.reply_text(
            ...
            "/deletevideo "
            "ANIME SEASON DUB EPISODE QUALITY\n\n"

            "Пример:\n"

            "/deletevideo "
            "mythic_spirit 1 anilibria 1 720"
